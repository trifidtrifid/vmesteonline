...
public class CloudTaggingTest {
	...
	public void testCreateMessage() {
		try {
			...
			subRubric.setParent(rootRubric.getKey().getId());
			pm.makePersistent(subRubric);
			subSubRubric.setParent(subRubric.getKey().getId());
			...
			msgRubric1.addMessage(msg);
			msgRubric1.addMessage(new Message(0L, "MSG2 content", msgRubrics));
			...
			for (long rId = 1; rId < 4; rId++) {
				try {
					//checkout rubric rId
					List<RubricTag> results = (List<RubricTag>) q.execute(rId);
					if (!results.isEmpty()) {
						for (RubricTag rubric : results) {
							...
							if (null == rc.getParent()) {
								...
								System.out.println("RUbric "
										+ rubric.getContent()
										+ " Contains message: " + msgIr.getContent());
							}
							for (Message nextMsg: rubric.getMsgs()){
								System.out.println("RUbric "
										+ rubric.getContent()
										+ " Contains message: " + nextMsg.getContent());
							}
						}
