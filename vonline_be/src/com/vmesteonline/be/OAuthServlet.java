...
public class OAuthServlet extends HttpServlet {
	...
	public void doGet(HttpServletRequest req, HttpServletResponse resp) throws IOException {
		...
		if (state.equals("google")) {
			...
			resp.getWriter().println("Hello, world " + jsonIdentity);
			try {
			...
			HashMap<String, String> respVals = new HashMap<String, String>();
			for (String data : pairs) {
				String[] p = data.split("=");
				if (p.length == 2) {
					respVals.put(p[0], p[1]);
				}
			}
			resp.getWriter().println("token is '" + response.toString() + "'");
...
//				resp.getWriter().println("<br><br>" + jsonObj.getString("access_token"));
				resp.getWriter().println("<br><br>" + jsonObj.getString("email"));
