define(
    ...
    function( $ ,flexslider, thriftModule,basketModule,spinnerModule,categoryModule,ordersModule ){
       ...
       /* var shopId = $('.shop').attr('id');
        ...
        function InitProductDetailPopup(selector){
            //try{
                selector.click(function(e,isHistoryNav){
                    ...
                    if(!boolPrepackRequired && product.weight){
                        popupHtml += '<span>Вес: '+ product.weight+'</span>';
                    }
                    if(boolPrepackRequired && product.minClientPack){
                        popupHtml += '<span>Мин.упаковка: '+ product.minClientPack+'</span>';
                    }
        ...
        function cleanURLByProduct(state){
            var currentHash = document.location.hash.split('#');
            currentHash.pop();
            currentHash = currentHash.join('#');
            if(currentHash == "") currentHash = " ";

            window.history.replaceState(state, null, currentHash);
        }
